...
import ca.gc.aafc.agent.api.mapper.AgentControlledVocabularyMapper;
...
public class AgentControlledVocabularyRepository extends DinaRepositoryV2<AgentControlledVocabularyDto, AgentControlledVocabulary> {

  // Bean does not exist with keycloak disabled.
  private final DinaAuthenticatedUser authenticatedUser;

  private final AgentControlledVocabularyService agentControlledVocabularyService;

  public AgentControlledVocabularyRepository(
    @NonNull AgentControlledVocabularyService dinaService,
    ...
    super(
      ...
      AgentControlledVocabularyMapper.INSTANCE,
      AgentControlledVocabularyDto.class,
      AgentControlledVocabulary.class,
      props, objMapper);
    this.authenticatedUser = authenticatedUser.orElse(null);
    this.agentControlledVocabularyService = dinaService;
  }

  @Override
  protected Link generateLinkToResource(AgentControlledVocabularyDto dto) {
    try {
      return linkTo(methodOn(AgentControlledVocabularyRepository.class).onFindOne(dto.getUuid().toString(), null)).withSelfRel();
    } catch (ResourceNotFoundException | ResourceGoneException e) {
      throw new RuntimeException(e);
    }
  }

  @GetMapping(AgentControlledVocabularyDto.TYPENAME + "/{idOrKey}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable String idOrKey, HttpServletRequest req)
    ...
    if (StringUtils.isBlank(idOrKey)) {
      throw ResourceNotFoundException.create(AgentControlledVocabularyDto.TYPENAME, "");
    }
    ...
    AgentControlledVocabulary entity = agentControlledVocabularyService.findOneByKey(idOrKey);
    if (entity == null) {
      throw ResourceNotFoundException.create(AgentControlledVocabularyDto.TYPENAME, idOrKey);
    }
    return handleFindOne(entity.getUuid(), req);
  }

  @PostMapping(path = AgentControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH,
  ...
  @GetMapping(AgentControlledVocabularyDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = AgentControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(AgentControlledVocabularyDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(path = AgentControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PatchMapping(AgentControlledVocabularyDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @DeleteMapping(path = AgentControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(AgentControlledVocabularyDto.TYPENAME + "/{id}")
  @Transactional
