...
import ca.gc.aafc.agent.api.mapper.IdentifierMapper;
...
public class IdentifierRepository extends DinaRepositoryV2<IdentifierDto, Identifier> {
  ...
  @Override
  protected Link generateLinkToResource(IdentifierDto dto) {
    try {
      return linkTo(methodOn(IdentifierRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  public ResponseEntity<RepresentationModel<?>> onBulkLoad(@RequestBody
      ...
      throws ResourceNotFoundException, ResourceGoneException, ResourcesNotFoundException, ResourcesGoneException {
