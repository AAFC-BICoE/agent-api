...
public class OrganizationRepository extends DinaRepositoryV2<OrganizationDto, Organization> {

  private static final String TYPE = OrganizationDto.TYPENAME;
  ...
  protected Link generateLinkToResource(OrganizationDto dto) {
    try {
      return linkTo(methodOn(OrganizationRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
